...
class InputHandle:
    ...
    def load(self):
        ...
        if self.num_paths == 2:
            ...
            num_clips_1 = dat_1['clips'].shape[0]
            dat_2['clips'][:, 0] += num_clips_1  # start frame adjustment for second dataset
            self.data['clips'] = np.concatenate((dat_1['clips'], dat_2['clips']), axis=0)
    ...
    def begin(self, do_shuffle=True):
        ...
        self.current_output_length = self.current_input_length
    ...
    def input_batch(self):
        ...
        for i in range(self.current_batch_size):
            ...
            begin = self.data['clips'][batch_ind, 0]
            end = self.data['clips'][batch_ind, 0] + self.data['clips'][batch_ind, 1]
    ...
    def output_batch(self):
        ...
        if self.is_output_sequence:
            output_dim = self.data['dims']
            output_batch = np.zeros((self.current_batch_size, self.current_output_length) + tuple(output_dim))
        else:
            output_batch = np.zeros((self.current_batch_size,) + tuple(self.data['dims']))
        ...
        for i in range(self.current_batch_size):
            ...
            if self.is_output_sequence:
                ...
                if data_slice.shape[0] == output_batch.shape[1]:
